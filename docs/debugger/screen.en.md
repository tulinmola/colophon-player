...
The addresses it walks are the board's own wiring: the low ten address lines of the 6845 land on A10 to A1, the raster line on A13 to A11, and the top two on A15 and A14. That is why a character row is scattered across eight blocks two kilobytes apart, and why a row's bytes wrap around inside their two-kilobyte slice instead of running on into the next one.
