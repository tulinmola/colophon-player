...
`<colophon-z80>` shows the processor: its registers in pairs with the shadow set beside them, the flags bit by bit, and the state that decides what it does when an interrupt arrives.
