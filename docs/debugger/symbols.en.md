...
The right button on a name opens [the breakpoint form](breakpoints.en.md#setting-one) with its address already in it, and offers to show that address in each [memory panel](memory.en.md#moving) in the same machine, under its label.
