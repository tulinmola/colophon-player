...
The debugger owns its page and uses the light DOM: a selection dragged across two shadow roots copies only the first, and ARIA relationships do not cross one either, and a debugger is made of both. The player returns later as a composition of the same viewers with a shadow root around it, not as a second application.
...
Views follow the chips, not the machine: one view per chip header, and a machine view only for what `cpc.c` itself owns — the memory map, the I/O decode, the video address wiring. A second machine sharing the Z80, the 6845, the AY and the 8255 inherits their views unchanged.

The module is fetched after the elements reach the page, so nothing may look for a machine once and expect to find it; absorbing that is the whole of what `Viewer` is for. Viewers find the machine by walking up for whatever holds one, never by tag name or attribute.

Viewers draw on the machine's events, never on a timer of their own, and write only what has changed. They build their markup once with `html` and never again: a second `innerHTML` throws the nodes away and takes with them the selection a reader was dragging and the focus of a field being edited. Values go into `[data-field]` nodes as text afterwards, which is why no machine data passes through a template and `html` needs no escaping.
