...
A value the reader can change is a form control, so the platform carries the editing: `change` is the commit and nothing else commits, and a value the `pattern` refuses is never written. The render leaves a focused field alone, so a half-typed value survives the machine's own updates, and it writes `defaultValue` too, so Escape reverts to the machine instead of blanking the field.
